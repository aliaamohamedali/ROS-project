...
def yolo_function(image):
    ...
    if len(idxs) > 0:
		...
		print("new object: %s "%(LABELS[class_ids[i]][:-1]))
...
def check_distance(x,y,w,h):
    ...
    if((depth_image[y,x]/10.0) <60.0 and (depth_image[y,x+w]/10.0)<60.0 and (depth_image[y+h,x]/10.0)<60.0
                and (depth_image[y+h,x+w]/10.0)<60.0 and (depth_image[y+(h/2),x+(w/2)]/10.0)<60.0):
                    return True
    else:
        return False
...
depth_image = 0
def get_distance(image_msg):
      global depth_image
      bridge=CvBridge()
      depth_image = bridge.imgmsg_to_cv2(image_msg, desired_encoding="32FC1")
...
rospy.Subscriber("/camera/depth_registered/image_raw",Image,get_distance)
